...
def main():
    ...
    with open('output.txt', 'w') as r:
        ...
        for i, ip in enumerate(IP):
            os.system('ping 8.8.8.8 -c 4 -I tun%s > ping.txt' % i)

            start_timeDn = time.time()
            os.system('sudo /home/oyuen/PycharmProjects/getNetStats/wget.sh --interface tun%s down' % i)
            ...
            dl_bw = float(100) / float(total_timeDn)
            dl_bw = "%.3f" % round(dl_bw, 3)
            print('Download Bandwidth: ', dl_bw, 'mbps')

            start_timeUp = time.time()
            os.system('sudo /home/oyuen/PycharmProjects/getNetStats/wget.sh --interface tun%s up' % i)
            ...
            up_bw = float(100) / float(total_timeUp)
            up_bw = "%.3f" % round(up_bw, 3)
